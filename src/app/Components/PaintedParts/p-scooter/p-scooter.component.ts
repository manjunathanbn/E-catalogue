...
@Component({
  selector: 'app-p-scooter',
  templateUrl: './p-scooter.component.html',
  styleUrls: ['./p-scooter.component.scss']
})
export class PScooterComponent implements OnInit {
  ...
  tabChanges($event){
    ...
    else
    {
      ...
      this.myThumbnail = this.filterInnerTab[0].image;
      this.myFullresImage = this.filterInnerTab[0].image;
    }
  ...
  getPaintMopedDetail() {
    ...
    this.masterServices.getReq('', 'api/Catalouge/GetPaintedModelDetails?Type=SCOOTER').subscribe(
      (resp: any) => 
     {
